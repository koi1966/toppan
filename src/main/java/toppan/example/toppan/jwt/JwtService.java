...
//                        .map(GrantedAuthority::getAuthority).findFirst().orElseThrow())
    ...
    public Optional<ParsedJwt> parseToken(final String jwtToken){

        try {
            final DecodedJWT decodedJWT = JWT.require(Algorithm.HMAC512(jwtProperties.getSecret()))
...
//        }
   ...
   public record ParsedJwt(String username, String role) { }
